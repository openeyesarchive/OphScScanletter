<?php 
class m130118_143751_event_type_OphScScanletter extends CDbMigration
{
	public function up() {
		...
		if (!$this->dbConnection->createCommand()->select('id')->from('element_type')->where('name=:name and event_type_id=:eventTypeId', array(':name'=>'Document',':eventTypeId'=>$event_type['id']))->queryRow()) {
			$this->insert('element_type', array('name' => 'Document','class_name' => 'Element_OphScScanletter_Document', 'event_type_id' => $event_type['id'], 'display_order' => 1));
		}
		...
		$this->createTable('et_ophscscanletter_document', array(
				...
				'PRIMARY KEY (`id`)',
				'KEY `et_ophscscanletter_document_lmui_fk` (`last_modified_user_id`)',
				'KEY `et_ophscscanletter_document_cui_fk` (`created_user_id`)',
				'KEY `et_ophscscanletter_document_ev_fk` (`event_id`)',
				'KEY `et_ophscscanletter_document_asset_id_fk` (`asset_id`)',
				'CONSTRAINT `et_ophscscanletter_document_lmui_fk` FOREIGN KEY (`last_modified_user_id`) REFERENCES `user` (`id`)',
				'CONSTRAINT `et_ophscscanletter_document_cui_fk` FOREIGN KEY (`created_user_id`) REFERENCES `user` (`id`)',
				'CONSTRAINT `et_ophscscanletter_document_ev_fk` FOREIGN KEY (`event_id`) REFERENCES `event` (`id`)',
				'CONSTRAINT `et_ophscscanletter_document_asset_id_fk` FOREIGN KEY (`asset_id`) REFERENCES `asset` (`id`)',
	...
	public function down() {
		...
		// --- drop element tables ---
		$this->dropTable('et_ophscscanletter_document');
		...
		// echo "m000000_000001_event_type_OphScScanletter does not support migration down.\n";
		// return false;
